...
(function () {
  ...
  var getComments = function () {
    var text = [];
    for (var i = 0; i < window.utils.getRandom(MIN, messages.length); i++) { // случайное кол-во комментариев
      text[i] = messages[window.utils.getRandom(MIN, messages.length)]; // заполняю случайнм комментарием
    }
    var comment = {
      avatar: avatars[window.utils.getRandom(MIN, avatars.length)],
      message: text,
      name: names[window.utils.getRandom(MIN, names.length)]
    };
    return comment;
  };
  ...
  var initPhoto = function () {
    ...
    for (var i = 0; i < PHOTOS_ON_PAGE; i++) {
      photos[i] = {
        url: 'photos/' + (i + 1) + '.jpg',
        likes: window.utils.getRandom(MIN_LIKES, MAX_LIKES),
